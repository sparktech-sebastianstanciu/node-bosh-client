...
var DirectorClient = module.exports = function (info) {
    ...
    this.takeSnapshot = function (deploymentName, opt, callback) {
        var options = {
            method: 'POST'
        },
            url = '';
        if (opt.job && opt.index) {
            url = this.urlFor(['deployments', deploymentName, 'jobs',
                opt.job, opt.index, 'snapshots']);
        } else {
            url = this.urlFor(['deployments', deploymentName, 'snapshots']);
        }
        options.url = url;
        this.requestAndTrack(options, callback);
    };
